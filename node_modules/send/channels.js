...
module.exports = async (session, message) => {
    if (await session.set()) {
        const { channel_id, text } = message;

        if (await channel.hasDMPermission(channel_id, session.user_id)) {
            if (typeof text === "string" && text.length > 0) {
                if (text.length <= 1024) {
                    const db = await require("class/database.js");
                    const result = await db.collection("messages").insertOne({ channel_id: ObjectId(channel.toObjectId(channel_id)), user_id: ObjectId(session.user_id), text: text, date: new Date() });
